...
- First test suite for the repo (14 tests): multi-container isolation, per-container Clear, tracking dedup, the loud-resolution contract, try-first precedence including pre-registered alternates and factories, same-scan collision fail-fast, and the scan exclusions.
